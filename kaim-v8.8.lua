-- ============================================================
--  KAIM v8.8  |  WindUI Edition (Singularity Revision)
...
-- Strict single-assignment pcalls to prevent Access Violations
local VirtualInput
...
-- ============================================================
-- Safe container: prefer gethui, then CoreGui, then PlayerGui
-- Never fall through to workspace (would replicate to server)
...
local function LoadWindUI()
    ...
    for _, url in ipairs(urls) do
        local loadOk, result = pcall(function() return loadstring(game:HttpGet(url))() end)
        if loadOk and result then
            return result
        elseif not loadOk then
            warn("KAIM | WindUI Fetch Error (" .. url .. "): " .. tostring(result))
        end
    end
    return nil
end
...
WindUI = LoadWindUI()
if not WindUI then
    error("KAIM | Failed to load WindUI. Check executor HTTP capabilities or internet connection.")
end
...
-- ============================================================
WindUI:AddTheme({
    Name = "KAIM Singularity",
    Accent = Color3.fromRGB(140, 82, 255),
    Background = Color3.fromRGB(8, 8, 10),
    BackgroundTransparency = 0,
    Outline = Color3.fromRGB(34, 34, 40),
    Text = Color3.fromRGB(234, 234, 234),
    Placeholder = Color3.fromRGB(102, 102, 102),
    Button = Color3.fromRGB(18, 18, 22),
    Icon = Color3.fromRGB(255, 255, 255),
    Hover = Color3.fromRGB(26, 26, 32),
    WindowBackground = Color3.fromRGB(6, 6, 8),
    WindowShadow = Color3.fromRGB(0, 0, 0),
    DialogBackground = Color3.fromRGB(8, 8, 10),
...
-- Optimized UpdateDraw: direct writes avoid pairs() overhead on hot path
...
-- NOTE: math.pow does NOT exist in Luau. Use the ^ operator instead.
...
-- GLOBALLY SHARED RAYCAST PARAMS (saves allocations per second)
...
-- FPS-adaptive frame budgeting
local _lastFrameTime    = 0
local _avgDeltaTime     = 0.016
local _fpsAdaptiveSkip  = 1  -- process every Nth player per frame when FPS is low

local thudLerpedHP      = 100
local thudAlpha         = 0
local thudLastTextCache = ""
local thudLastNameCache = ""
local _hudLastVisible   = false  -- FIX: guard redundant hide calls
...
-- Continuous Gradient Color Lerping (O(1) Matrix Map)
...
local function UpdateCharCache(player, char)
    ...
    task.spawn(function()
        ...
        if hrp and head and hum then
            ...
            cData.VelHistory = {}
            cData._velRing   = {Vector3New(), Vector3New(), Vector3New()}  -- FIX: circular ring
            cData._velIdx    = 1
...
local function CreateESP(player)
    local esp = {
        ...
        -- Arrow lines stored in a table for easy iteration
        ArrowLines = {},
        Highlight  = nil,
        _isVisible   = false,
        _lastVisible = false,
        ...
        _trackedHP   = 100,
        _lastFont    = -1,  -- FIX: cache font to avoid per-frame iteration
        -- String GC Cache
        _lastTextCase  = "",
        _lastDistStrInt = -1,
        _lastHPStrInt   = -1,
        ...
        _lastNameStr    = "\0",
        _lastUnameStr   = "\0",
        -- Extents cache to avoid GetExtentsSize every frame
        _cachedExtentsY = 4,
        _cachedExtentsX = 2,
    ...
    -- FIX: Arrow lines in a table instead of flat ArrowL1/L2/L3/L4 names
    for i = 1, 4 do
...
tableInsert(KaimConnections, Players.PlayerRemoving:Connect(function(player)
    ...
    if esp then
        task.defer(function() DestroyESP(esp) end)
    end
...
-- FIX: Single CharacterAdded connection for local player (was duplicated)
tableInsert(KaimConnections, LocalPlayer.CharacterAdded:Connect(function(c)
    UpdateCharCache(LocalPlayer, c)
    -- Rebuild noclip cache if noclip is active
...
local function HideTHUD()
    -- FIX: Only update if THUD was previously visible
...
local function UpdateAimlock(camPos, screenWidth, screenHeight, deltaTime, aimSet, avoidSet, visSet, fovSet, fovPos, activeCam)
    ...
    if aimSet.Enabled and aimSet.IsAiming and not aimSet.PeriodicDisable then
        if mathRandom(1, 100) > aimSet.HitChance then
            ...
            -- FIX: Properly nil-guard Hum before accessing Health
            ...
            if targetPart then
                ...
                if aimSet.SmoothAiming then
                    ...
                    if smoothFactor < 1 then
                        ...
                        -- FIX: use ^ instead of deprecated math.pow
                ...
                if visSet.TargetUI then
                    ...
                    -- FIX: guard against NaN from bad health values
                    ...
                    -- Clear extra elements that vary by style
                    UpdateDraw(THUD.Shadow,  {Visible = false})
                    UpdateDraw(THUD.Outline, {Visible = false})
                    UpdateDraw(THUD.BG,      {Visible = false})
                    UpdateDraw(THUD.Accent2, {Visible = false})
...
local function UpdateESP(camPos, screenWidth, screenHeight, visSet, activeCam)
    ...
    local processed = 0
    for i = 1, #PlayerCache do
        -- FPS-adaptive frame budgeting: skip players when FPS is low
        if frameSkip and processed >= maxPerFrame and _avgDeltaTime > 0.02 then
            -- Over budget (below ~50 FPS) — defer remaining players to next frame
            local player = PlayerCache[i]
            local esp = ESPObjects[player]
            if esp and esp._lastVisible then
                -- Don't hide — just skip update this frame, keeps last-drawn state
            end
            continue
        end
        ...
        local cData = CharCache[player]
        -- Safety: pcall-guard HRP.Parent access to prevent crash on destroyed instances
        ...
        -- LOD: far players get simplified ESP (box+name only, no raycasts/tracers/weapon)
        ...
        -- Skip expensive visibility raycasts for LOD players
        ...
        elseif isLOD then
            -- LOD players always use the static/vis color without raycast
            esp._isVisible = true
        else
            esp._isVisible = true
        end
        ...
        -- Skip chams for LOD players (highlights are expensive)
        ...
        -- Skip damage tracking for LOD players
        ...
        -- FIX: Staggered GetExtentsSize — only recompute every N frames per player
        ...
        -- FIX: Cache font to avoid per-frame iteration over 5 text objects
...
local function HeartbeatLoop(deltaTime)
    ...
    -- Kinematics Tracking — FIX: Circular ring buffer instead of insert-at-1
    local now = osClock()
    for _, player in ipairs(PlayerCache) do
        local cData = CharCache[player]
        if cData and cData.HRP then
            ...
            if dt > 0 and dt < 0.1 then
                ...
                -- Average ring without table allocation
...
-- ============================================================
--  WINDUI MENU BUILDING (Singularity v8.8)
-- ============================================================
local Window = WindUI:CreateWindow({
    Title     = "KAIM v8.8",
    Author    = "by FRK (Singularity Revision)",
    Folder    = "Kaim",
    Size      = UDim2.fromOffset(680, 600),
    Theme     = "KAIM Singularity",
    Resizable = true,
})
Window:SetToggleKey(nil)

-- FIX: Auto-save config when window is closed
Window:OnClose(function()
    local ConfigManager = Window.ConfigManager
    local currentCfg    = Window.CurrentConfig
    if currentCfg then
        pcall(function() currentCfg:Save() end)
    end
end)
...
--  [ DASHBOARD ]
-- ============================================================
local HomeWelcome = Tabs.Home:Section({ Title = "Welcome to KAIM v8.8", Box = true, Opened = true })
HomeWelcome:Paragraph({
    Title = "The Singularity Revision",
    Desc  = "v8.8: FPS-adaptive frame budgeting, LOD system for distant ESP, zero-allocation hot-path UpdateDraw, pcall-guarded HRP access, green visibility color, and comprehensive UI controls for every setting.",
    Icon  = "crown",
})

local HomeStat = Tabs.Home:Section({ Title = "Quick Stats", Box = true, Opened = true })
local fpsLabel = HomeStat:Paragraph({ Title = "FPS", Desc = "Measuring...", Icon = "activity" })
local playerLabel = HomeStat:Paragraph({ Title = "Players", Desc = "0 tracked", Icon = "users" })

-- Live FPS + player count updater (every 2 seconds)
task.spawn(function()
    while _env.KAIM_LOADED do
        local fps = mathFloor(1 / mathMax(_avgDeltaTime, 0.001))
        pcall(function() fpsLabel:SetDesc(fps .. " FPS  |  " .. string.format("%.1f", _avgDeltaTime * 1000) .. "ms frame time") end)
        pcall(function() playerLabel:SetDesc(#PlayerCache .. " players tracked") end)
        task.wait(2)
    end
end)
...
AimCore:Toggle({ Title = "Wall Check",         Desc = "Only lock visible targets.",                    Flag = "AimlockWallCheck", Value = true, Callback = function(v) Settings.Aimlock.WallCheck  = v end })
AimCore:Toggle({ Title = "Team Check",         Desc = "Ignore teammates.",                             Flag = "AimlockTeamCheck", Value = true, Callback = function(v) Settings.Aimlock.TeamCheck  = v end })
AimCore:Slider({ Title = "Hit Chance %",       Desc = "Probability of registering a lock each frame.", Flag = "AimlockHitChance", Step = 1, Value = {Min=1,Max=100,Default=100}, Callback = function(v) Settings.Aimlock.HitChance = v end })

local AimSetup = Tabs.Combat:Section({ Title = "Targeting Setup", Box = true, Opened = false })
...
AimSetup:Dropdown({ Title = "Aim Mode",        Desc = "Smart: best visible part.\nChaos: jitters to bypass ACs.", Flag = "AimlockAimMode", Values = {"Smart","Chaos","Head","Torso","Limbs","HRP"}, Value = "Smart", Callback = function(v) Settings.Aimlock.AimMode = v end })

-- FOV Controls
local FOVSec = Tabs.Combat:Section({ Title = "FOV Circle", Box = true, Opened = false })
FOVSec:Toggle({ Title = "Show FOV Circle",   Desc = "Displays the active locking radius.",          Flag = "FOVVisible",   Value = true, Callback = function(v) Settings.FOV.Visible      = v end })
FOVSec:Toggle({ Title = "Follow Cursor",     Desc = "FOV circle tracks mouse position.",            Flag = "FOVFollow",    Value = true, Callback = function(v) Settings.FOV.FollowCursor = v end })
FOVSec:Slider({ Title = "FOV Radius",        Desc = "Size of the targeting circle in pixels.",      Flag = "FOVRadius",    Step = 5,    Value = {Min=20,Max=600,Default=150}, Callback = function(v) Settings.FOV.Radius    = v end })
FOVSec:Slider({ Title = "FOV Thickness",     Desc = "Border thickness of the circle.",              Flag = "FOVThickness", Step = 0.5,  Value = {Min=0.5,Max=5,Default=1.5},  Callback = function(v) Settings.FOV.Thickness  = v end })
FOVSec:Toggle({ Title = "Pulse Animation",   Desc = "Pulsing radius animation on the FOV ring.",   Flag = "FOVPulse",     Value = false, Callback = function(v) Settings.FOV.Pulse = v end })
FOVSec:Toggle({ Title = "Filled FOV",        Desc = "Fill the FOV circle with a transparent tint.", Flag = "FOVFilled",    Value = false, Callback = function(v) Settings.FOV.Filled = v end })
FOVSec:Colorpicker({ Title = "FOV Color",    Desc = "Color of the FOV ring.",                       Flag = "FOVColor",     Default = COLOR_WHITE, Transparency = 0, Callback = function(v) Settings.FOV.Color = v end })

local PredSec = Tabs.Combat:Section({ Title = "3rd-Order Kinematics", Box = true, Opened = false })
PredSec:Toggle({ Title = "Enable Prediction",          Desc = "Leads aim based on enemy velocity.",                  Flag = "AimlockPredEnabled",   Value = true,  Callback = function(v) Settings.Aimlock.PredictionEnabled    = v end })
PredSec:Toggle({ Title = "Dynamic Prediction",         Desc = "Scales lead time with target distance.",              Flag = "AimlockDynPred",       Value = true,  Callback = function(v) Settings.Aimlock.DynamicPrediction     = v end })
PredSec:Toggle({ Title = "Jerk Prediction (3rd Order)",Desc = "Predicts sudden acceleration changes.",               Flag = "AimlockJerk",          Value = true,  Callback = function(v) Settings.Aimlock.JerkPrediction        = v end })
PredSec:Toggle({ Title = "Bullet Drop Compensation",   Desc = "Accounts for gravity on physical projectiles.",       Flag = "AimlockDrop",          Value = false, Callback = function(v) Settings.Aimlock.BulletDropCompensation = v end })
PredSec:Slider({ Title = "Prediction Strength",        Desc = "Base velocity multiplier.",                           Flag = "AimlockPrediction",    Step = 0.005, Value = {Min=0,Max=0.3,Default=0.135},   Callback = function(v) Settings.Aimlock.Prediction      = v end })
-- NEW: Strafe Prediction slider (was missing)
PredSec:Slider({ Title = "Strafe Multiplier",          Desc = "Scales lateral velocity prediction.",                 Flag = "AimlockStrafePred",    Step = 0.05,  Value = {Min=0,Max=3,Default=1.0},        Callback = function(v) Settings.Aimlock.StrafePrediction = v end })
PredSec:Slider({ Title = "Projectile Speed (m/s)",     Desc = "Estimated bullet speed for dynamic lead time.",       Flag = "AimlockProjSpeed",     Step = 100,   Value = {Min=100,Max=5000,Default=1000},   Callback = function(v) Settings.Aimlock.ProjectileSpeed  = v end })

local SmoothSec = Tabs.Combat:Section({ Title = "Humanization", Box = true, Opened = false })
SmoothSec:Toggle({ Title = "Smooth Tracking",      Desc = "Lerps camera instead of snapping.", Flag = "AimlockSmooth",    Value = false, Callback = function(v) Settings.Aimlock.SmoothAiming      = v end })
SmoothSec:Toggle({ Title = "Dynamic Smoothing",    Desc = "Scales smooth factor by distance to target on-screen.", Flag = "AimlockDynSmooth", Value = false, Callback = function(v) Settings.Aimlock.DynamicSmoothing   = v end })
SmoothSec:Toggle({ Title = "Micro-Tremor (Jiggle)", Desc = "Adds Perlin noise to bypass smoothing heuristics.",  Flag = "AimlockHumanize",  Value = false, Callback = function(v) Settings.Aimlock.HumanizeSmoothing  = v end })
SmoothSec:Slider({ Title = "Smooth Speed",         Desc = "Camera lerp rate.",                 Flag = "AimlockSmoothSpeed",Step=0.05, Value={Min=0.05,Max=1.0,Default=0.3}, Callback = function(v) Settings.Aimlock.SmoothSpeed = v end })
SmoothSec:Toggle({ Title = "Perlin Noise",         Desc = "Applies directional jitter to aimpos.", Flag="AimlockPerlin", Value=false, Callback=function(v) Settings.Aimlock.PerlinNoise=v end })
SmoothSec:Slider({ Title = "Noise Speed",          Desc = "Speed of the Perlin noise oscillation.", Flag="AimlockNoiseSpd", Step=0.1, Value={Min=0.1,Max=5,Default=1.0}, Callback=function(v) Settings.Aimlock.NoiseSpeed=v end })
SmoothSec:Slider({ Title = "Noise Amount",         Desc = "Magnitude of Perlin displacement (studs).", Flag="AimlockNoiseAmt", Step=0.05, Value={Min=0,Max=2,Default=0.5}, Callback=function(v) Settings.Aimlock.NoiseAmount=v end })

local OffsetSec = Tabs.Combat:Section({ Title = "Aim Offset", Box = true, Opened = false })
OffsetSec:Slider({ Title = "Offset X", Desc = "Horizontal aimpoint offset (studs).", Flag="AimOffX", Step=0.1, Value={Min=-5,Max=5,Default=0}, Callback=function(v) Settings.Aimlock.OffsetX=v end })
OffsetSec:Slider({ Title = "Offset Y", Desc = "Vertical aimpoint offset (studs).",   Flag="AimOffY", Step=0.1, Value={Min=-5,Max=5,Default=0}, Callback=function(v) Settings.Aimlock.OffsetY=v end })
OffsetSec:Slider({ Title = "Offset Z", Desc = "Depth aimpoint offset (studs).",      Flag="AimOffZ", Step=0.1, Value={Min=-5,Max=5,Default=0}, Callback=function(v) Settings.Aimlock.OffsetZ=v end })
...
TBotCore:Toggle({ Title = "Team Check",           Desc = "Don't fire at teammates.",                    Flag="TBTeamCheck",Value=true,  Callback=function(v) Settings.Triggerbot.TeamCheck   = v end })
TBotCore:Toggle({ Title = "Spherecast (Thick Ray)",Desc = "Forgiving thick-ray for fast targets.",      Flag="TBSphere",   Value=true,  Callback=function(v) Settings.Triggerbot.Spherecast  = v end })
TBotCore:Slider({ Title = "Ray Thickness",        Desc = "Width of the spherecast.",                   Flag="TBThick",    Step=0.1, Value={Min=0.1,Max=3.0,Default=0.5}, Callback=function(v) Settings.Triggerbot.Thickness = v end })
TBotCore:Slider({ Title = "Trigger Delay",        Desc = "Humanization delay before firing (seconds).", Flag="TBDelay",    Step=0.01,Value={Min=0.01,Max=0.5,Default=0.05}, Callback=function(v) Settings.Triggerbot.Delay = v end })
TBotCore:Slider({ Title = "Hit Chance %",         Desc = "Probability of trigger firing per check.",    Flag="TBHitChance",Step=1, Value={Min=1,Max=100,Default=100}, Callback=function(v) Settings.Triggerbot.HitChance = v end })

local HitboxCore = Tabs.Combat:Section({ Title = "Hitbox Override", Box = true, Opened = false })
HitboxCore:Toggle({ Title = "Enable Hitbox Expander", Desc = "Increases size of enemy hitboxes.",     Flag="HitboxEnabled", Value=false, Callback=function(v) Settings.Hitbox.Enabled     = v end })
...
DetectionSec:Toggle({ Title = "Periodic Aim Disable", Desc = "Randomly drops tracking to look legitimate.", Flag="PeriodicDisable", Value=false, Callback=function(v) Settings.DetectionAvoidance.PeriodicAimDisable = v end })
DetectionSec:Slider({ Title = "Disable Chance",       Desc = "Probability of tracking loss per cycle.",     Flag="DisableChance",   Step=0.01, Value={Min=0.01,Max=1.0,Default=0.1}, Callback=function(v) Settings.DetectionAvoidance.DisableChance   = v end })
DetectionSec:Slider({ Title = "Disable Duration",     Desc = "How long each disable cycle lasts (s).",     Flag="DisableDuration", Step=0.05, Value={Min=0.05,Max=1.0,Default=0.2}, Callback=function(v) Settings.DetectionAvoidance.DisableDuration = v end })
...
ESPCore:Toggle({ Title = "Team Check",      Desc = "Respect team colors and filtering.",     Flag="ESPTeamCheck",  Value=true,  Callback=function(v) Settings.Visuals.TeamCheck       = v end })
ESPCore:Toggle({ Title = "Show Teammates",  Desc = "Render ESP on allied players.",         Flag="ESPShowTeam",   Value=false, Callback=function(v) Settings.Visuals.ShowTeammates   = v end })
ESPCore:Colorpicker({ Title = "Teammate Color", Desc = "Box/text color for teammates.", Flag="ESPTeamColor", Default=Color3RGB(0,200,255), Transparency=0, Callback=function(v) Settings.Visuals.TeammateColor = v end })
...
BoxSec:Dropdown({ Title = "Box Style",      Desc = "Full wraps vs Corner bracket style.",         Flag="ESPBoxStyle",    Values={"Standard","Corner"}, Value="Corner", Callback=function(v) Settings.Visuals.ESPBoxStyle     = v end })
BoxSec:Toggle({ Title = "Box Outline",      Desc = "Dark outline behind box lines.",              Flag="ESPOutline",     Value=true,  Callback=function(v) Settings.Visuals.ESPOutline      = v end })
BoxSec:Toggle({ Title = "Box Fill",         Desc = "Semi-transparent fill inside bounding box.", Flag="ESPBoxFill",     Value=false, Callback=function(v) Settings.Visuals.ESPBoxFill      = v end })
BoxSec:Slider({ Title = "Fill Transparency",Desc = "Opacity of the box fill.",                   Flag="ESPBoxFillTrans",Step=0.05, Value={Min=0,Max=1,Default=0.2}, Callback=function(v) Settings.Visuals.ESPBoxFillTrans = v end })
BoxSec:Toggle({ Title = "Depth Scaling",    Desc = "Lines thin out as targets get further away.",Flag="ESPDynThick",    Value=true,  Callback=function(v) Settings.Visuals.DynamicThickness= v end })
BoxSec:Slider({ Title = "Base Thickness",   Desc = "Base drawing API line thickness.",            Flag="ESPBaseThick",   Step=0.5,  Value={Min=0.5,Max=3,Default=1.5}, Callback=function(v) Settings.Visuals.BaseThickness    = v end })
...
TextSec:Dropdown({ Title = "Name Style",    Desc = "Which name to display.",                      Flag="ESPNameStyle",   Values={"Display Name","Username","Both"}, Value="Display Name", Callback=function(v) Settings.Visuals.ESPNameStyle    = v end })
TextSec:Dropdown({ Title = "Text Casing",   Desc = "Formatting wrapper for all text.",            Flag="ESPTextCase",    Values={"Normal","UPPERCASE"}, Value="UPPERCASE", Callback=function(v) Settings.Visuals.TextCase         = v end })
...
TextSec:Toggle({ Title = "Show Weapon",     Desc = "Detects currently held Tool.",                Flag="ESPWeapon",      Value=false, Callback=function(v) Settings.Visuals.WeaponESP         = v end })
TextSec:Toggle({ Title = "Numeric Health",  Desc = "Renders HP text under bounding box.",        Flag="ESPHealthNums",  Value=false, Callback=function(v) Settings.Visuals.HealthNumbers     = v end })

local IndicatorSec = Tabs.Visuals:Section({ Title = "Indicators & Tracers", Box = true, Opened = false })
IndicatorSec:Toggle({ Title = "Health Bar",         Desc = "Gradient health bar on the left of each box.",  Flag="ESPHealthBar",  Value=true,  Callback=function(v) Settings.Visuals.HealthBar      = v end })
-- NEW: Tracer Lines toggle (was missing from original UI)
IndicatorSec:Toggle({ Title = "Tracer Lines",       Desc = "Lines from screen edge to each enemy.",         Flag="TracerLines",   Value=false, Callback=function(v) Settings.Visuals.TracerLines     = v end })
IndicatorSec:Dropdown({ Title = "Tracer Origin",    Desc = "Where tracers start from.",                     Flag="TracerOrigin",  Values={"Bottom","Center","Top"}, Value="Bottom", Callback=function(v) Settings.Visuals.TracerOrigin    = v end })
IndicatorSec:Colorpicker({ Title = "Tracer Color",  Desc = "Color of tracer lines.",                        Flag="TracerColor",   Default=Color3RGB(0,255,100), Transparency=0, Callback=function(v) Settings.Visuals.TracerColor     = v end })
IndicatorSec:Toggle({ Title = "Look Tracers",       Desc = "Lines indicating enemy look direction.",        Flag="LookTracers",   Value=false, Callback=function(v) Settings.Visuals.LookTracers     = v end })
IndicatorSec:Slider({ Title = "Look Tracer Length", Desc = "Length of the look tracer line (studs).",      Flag="LookTracerLen", Step=1, Value={Min=1,Max=30,Default=5}, Callback=function(v) Settings.Visuals.LookTracerLength = v end })
IndicatorSec:Toggle({ Title = "Off-Screen Arrows",  Desc = "Arrow indicators for enemies behind camera.",  Flag="OffScreen",     Value=false, Callback=function(v) Settings.Visuals.OffScreenArrows  = v end })
IndicatorSec:Toggle({ Title = "Damage Numbers",     Desc = "Physics-based hit numbers on damage.",         Flag="DamageNumbers", Value=false, Callback=function(v) Settings.Visuals.DamageNumbers    = v end })

local HUDSection = Tabs.Visuals:Section({ Title = "Target HUD", Box = true, Opened = false })
HUDSection:Toggle({ Title = "Show Target HUD",  Desc = "Displays intel of your current aimlock target.",  Flag="HUDEnabled",  Value=true,  Callback=function(v) Settings.Visuals.TargetUI      = v end })
HUDSection:Dropdown({ Title = "HUD Style",      Desc = "Architectural style of the target interface.",    Flag="HUDStyle",    Values={"Ascension","Valorant","Standard","Cyber","Minimal","Tech","Apex"}, Value="Ascension", Callback=function(v) Settings.Visuals.TargetUIStyle = v end })
HUDSection:Slider({ Title = "HUD Scale",        Desc = "Scale multiplier for the target HUD.",           Flag="HUDScale",    Step=0.05, Value={Min=0.5,Max=2.0,Default=1.0}, Callback=function(v) Settings.Visuals.TargetUIScale = v end })

local ESPColors = Tabs.Visuals:Section({ Title = "Colors", Box = true, Opened = false })
ESPColors:Toggle({ Title = "Visibility Check Colors", Desc = "Colors shift if the enemy is behind a wall.", Flag="ESPVisColors",   Value=true,  Callback=function(v) Settings.Visuals.UseVisColors  = v end })
ESPColors:Colorpicker({ Title = "Visible Color",       Desc = "Color when enemy is visible.",              Flag="ESPVisColor",    Default=Color3RGB(0,255,100), Transparency=0, Callback=function(v) Settings.Visuals.VisColor      = v end })
ESPColors:Colorpicker({ Title = "Hidden Color",        Desc = "Color when enemy is behind a wall.",        Flag="ESPHiddenColor", Default=COLOR_RED,            Transparency=0, Callback=function(v) Settings.Visuals.HiddenColor   = v end })
ESPColors:Colorpicker({ Title = "Static Box Color",    Desc = "Color when visibility check is disabled.",  Flag="ESPStaticColor", Default=COLOR_WHITE,          Transparency=0, Callback=function(v) Settings.Visuals.StaticBoxColor = v end })
ESPColors:Colorpicker({ Title = "Damage Impact Color", Desc = "Color of damage number text.",              Flag="DamageColor",    Default=Color3RGB(255,255,0),  Transparency=0, Callback=function(v) Settings.Visuals.DamageColor   = v end })
ESPColors:Toggle({ Title = "Custom Name Color",        Desc = "Override the name color.",                  Flag="ESPCustomName",  Value=false, Callback=function(v) Settings.Visuals.UseCustomNameColor = v end })
ESPColors:Colorpicker({ Title = "Name Color",          Desc = "Custom color for display names.",           Flag="ESPNameColor",   Default=COLOR_WHITE, Transparency=0, Callback=function(v) Settings.Visuals.ESPNameColor = v end })

local ESPChams = Tabs.Visuals:Section({ Title = "Chams Overlay", Box = true, Opened = false })
ESPChams:Toggle({ Title = "Enable Highlights",   Desc = "Renders engine overlays on character bodies.", Flag="ChamsEnabled", Value=false, Callback=function(v) Settings.Visuals.ChamsEnabled     = v end })
ESPChams:Toggle({ Title = "Use Visibility Colors",Desc = "Chams change color when hidden by walls.",   Flag="ChamsVisColor",Value=true,  Callback=function(v) Settings.Visuals.ChamsUseVisColors = v end })
ESPChams:Colorpicker({ Title = "Fill Color",     Desc = "Cham fill color (when UseVisColors is off).",  Flag="ChamsFill",    Default=COLOR_WHITE, Transparency=0, Callback=function(v) Settings.Visuals.ChamsFillColor   = v end })
ESPChams:Colorpicker({ Title = "Outline Color",  Desc = "Cham outline color.",                         Flag="ChamsOutline", Default=COLOR_WHITE, Transparency=0, Callback=function(v) Settings.Visuals.ChamsOutlineColor = v end })
ESPChams:Slider({ Title = "Fill Transparency",   Desc = "Transparency of cham fill.",                  Flag="ChamsFillTrans",  Step=0.05, Value={Min=0,Max=1,Default=0.5}, Callback=function(v) Settings.Visuals.ChamsFillTrans    = v end })
ESPChams:Slider({ Title = "Outline Transparency",Desc = "Transparency of cham outline.",               Flag="ChamsOutTrans",   Step=0.05, Value={Min=0,Max=1,Default=0},   Callback=function(v) Settings.Visuals.ChamsOutlineTrans  = v end })
...
WorldCore:Slider({ Title = "Time of Day",         Desc = "0-24 hour clock.",                 Flag="WorldTime",     Step=0.5,  Value={Min=0,Max=24,Default=14}, Callback=function(v) Settings.World.Time          = v end })
WorldCore:Slider({ Title = "Brightness",          Desc = "Scene brightness level.",           Flag="WorldBrightness",Step=0.1, Value={Min=0,Max=5,Default=2},  Callback=function(v) Settings.World.Brightness    = v end })
WorldCore:Toggle({ Title = "Global Shadows",      Desc = "Enable or disable global shadows.", Flag="WorldShadows",  Value=false, Callback=function(v) Settings.World.GlobalShadows = v end })
WorldCore:Colorpicker({ Title = "Ambient Light",  Desc = "Scene ambient light color.",        Flag="WorldAmbient",  Default=COLOR_WHITE, Transparency=0, Callback=function(v) Settings.World.Ambient = v end })
...
MovementSec:Slider({ Title = "Walk Speed",        Desc = "Target walk speed.",                 Flag="WalkSpeed",        Step=1, Value={Min=5,Max=100,Default=16},  Callback=function(v) Settings.Player.WalkSpeed        = v end })
MovementSec:Toggle({ Title = "Force Jump Power",  Desc = "Overrides server-sided jump height.", Flag="JumpPowerEnabled", Value=false, Callback=function(v) Settings.Player.JumpPowerEnabled = v end })
MovementSec:Slider({ Title = "Jump Power",        Desc = "Target jump power.",                 Flag="JumpPower",        Step=5, Value={Min=10,Max=250,Default=50}, Callback=function(v) Settings.Player.JumpPower        = v end })
MovementSec:Toggle({ Title = "Camera FOV Override",Desc = "Override camera field of view.",    Flag="CamFOVEnabled",    Value=false, Callback=function(v) Settings.Player.CameraFOVEnabled = v end })
MovementSec:Slider({ Title = "Camera FOV",         Desc = "Field of view in degrees.",         Flag="CamFOV",           Step=1, Value={Min=30,Max=120,Default=70}, Callback=function(v) Settings.Player.CameraFOV        = v end })
...
-- FIX: Auto-setup the default config object so OnClose save works
Window.CurrentConfig = ConfigManager:Config(ConfigName)

local ConfigNameInput = ConfigSection:Input({ Title = "Config Name", Icon = "file-cog", Value = ConfigName, Callback = function(value) ConfigName = value end })
...
ConfigSection:Button({ Title = "Save State", Justify = "Center", Icon = "save", Callback = function()
    Window.CurrentConfig = ConfigManager:Config(ConfigName)
    if Window.CurrentConfig:Save() then
        WindUI:Notify({ Title = "Saved", Content = ConfigName .. ".json saved.", Icon = "check", Duration = 3 })
    end
    AllConfigsDropdown:Refresh(ConfigManager:AllConfigs())
end })

-- FIX: Load button now uses :Config() not :CreateConfig() so it loads existing file
ConfigSection:Button({ Title = "Load State", Justify = "Center", Icon = "folder-open", Callback = function()
    Window.CurrentConfig = ConfigManager:Config(ConfigName)
    if Window.CurrentConfig:Load() then
        WindUI:Notify({ Title = "Loaded", Content = ConfigName .. ".json loaded.", Icon = "refresh-cw", Duration = 3 })
    end
end })

ConfigSection:Button({ Title = "Delete State", Justify = "Center", Icon = "trash-2", Callback = function()
    local cfg = ConfigManager:Config(ConfigName)
    cfg:Delete()
    AllConfigsDropdown:Refresh(ConfigManager:AllConfigs())
    WindUI:Notify({ Title = "Deleted", Content = ConfigName .. ".json deleted.", Icon = "trash", Duration = 3 })
end })

-- Performance Section
local PerfSection = Tabs.Settings:Section({ Title = "Performance (Lag Reduction)", Box = true, Opened = true })
PerfSection:Toggle({ Title = "FPS-Adaptive Frame Skip", Desc = "When FPS drops below 50, limits ESP updates per frame to reduce lag.", Flag="PerfFrameSkip", Value=true, Callback=function(v) Settings.Performance.FrameSkip = v end })
PerfSection:Slider({ Title = "Max ESP Per Frame",        Desc = "Max players to render ESP for in a single frame (lower = better FPS).", Flag="PerfMaxESP", Step=1, Value={Min=5,Max=50,Default=20}, Callback=function(v) Settings.Performance.MaxESPPerFrame = v end })
PerfSection:Slider({ Title = "LOD Distance (studs)",     Desc = "Beyond this, players get simplified ESP (no tracers, chams, weapon). Saves heavy draw calls.", Flag="LODDistance", Step=25, Value={Min=100,Max=1000,Default=500}, Callback=function(v) Settings.Visuals.LODDistance = v end })
PerfSection:Paragraph({
    Title = "How LOD Works",
    Desc  = "Players closer than LOD Distance get full ESP (tracers, chams, weapon, look tracers, damage numbers). Players further away get only box + name + health bar. This dramatically reduces lag in crowded servers.",
    Icon  = "info",
})
...
DangerSection:Button({
    ...
    Callback = function()
        ...
        -- Destroy tracers and look-tracers
        ...
        -- Remove FOV + HUD drawing objects
        ...
        -- FIX: Clear KAIM_LOADED so the script can be reloaded
        _env.KAIM_LOADED = false
        _G._KaimBuildNoclipCache = nil

        Window:Destroy()
        WindUI:Notify({ Title = "KAIM Unloaded", Content = "All systems shut down. Reload safely.", Icon = "shield-off", Duration = 4 })
    end
})
...
local function ToggleUI()
    if not kaimScreenGui or not kaimScreenGui.Parent then
        local containers = { SafeContainer, pcall(function() return game:GetService("CoreGui") end) and game:GetService("CoreGui") or nil, LocalPlayer:FindFirstChild("PlayerGui") }
...
tableInsert(KaimConnections, UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    -- FIX: pcall Enum.KeyCode lookup to prevent crash on invalid key string
...
Tabs.Home:Select()
WindUI:Notify({ Title = "KAIM v8.8 Singularity", Content = "Initialization complete. Press K to toggle.", Duration = 5, Icon = "shield-check" })
