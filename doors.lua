...
BypassEntityBox:AddToggle('AntiTimothy',{
Text = "Anti Timothy",
Default = false,
Callback = function(Value)
end
...
-- [SUPER OPTIMIZED] Smart Room Processing to completely eliminate 30s lag spikes
local function processPart(v)
local vName = v.Name
    
    if HidingPlaces[vName] then table.insert(Closets, v) end
    if Toggles.Ladder and Toggles.Ladder.Value and vName == "Ladder" then AddESP(v, "Ladder", LadderColor) end
    if Toggles.Fuse and Toggles.Fuse.Value and vName == "FuseObtain" then AddESP(v, "Fuse", FuseColor) end
    
    if Toggles.AntiLag and Toggles.AntiLag.Value and v:IsA("BasePart") then
        v:SetAttribute("Mat", v.Material)
        v.Material = Enum.Material.Plastic
    end
    
    if Toggles.InfItems and Toggles.InfItems.Value and v:IsA("ProximityPrompt") then
        task.spawn(function()
            task.wait(0.1)
            if v.Parent and (Names[v.Parent.Name] or vName == "FusesPrompt" or (v.Parent.Parent and v.Parent.Parent.Name == "Locker_Small_Locked")) then
                table.insert(Stored, v)
            end
        end)
    end
    
    if Toggles.DeleteSeekFE and Toggles.DeleteSeekFE.Value and vName == "TriggerEventCollision" then
        task.spawn(function()
            Notify("Deleting Seek", 3)
    ...
    if Toggles.SeekObf and Toggles.SeekObf.Value and (vName == "Seek_Arm" or vName == "ChandelierObstruction") then
        task.spawn(function()
            task.wait(0.1)
    ...
    if Toggles.Breakers and Toggles.Breakers.Value and vName == "LiveBreakerPolePickup" then AddESP(v, "Breaker", BreakerColor) end
    ...
    if Toggles.AntiSeekFlood and Toggles.AntiSeekFlood.Value and vName == "SeekFloodline" then v.CanCollide = true end
    if Toggles.ShowPath and Toggles.ShowPath.Value and vName == "SeekGuidingLight" then ShowSeekPath(v) end
    if Toggles.DeleteFigureFE and Toggles.DeleteFigureFE.Value and (vName == "FigureRig" or vName == "FigureRagdoll") then table.insert(Figures, v) end
    
    if Toggles.PromptClip and Toggles.PromptClip.Value and v:IsA("ProximityPrompt") then
        v:SetAttribute("Clip", v.RequiresLineOfSight)
        v.RequiresLineOfSight = false
    end
    
    if Toggles.Gold and Toggles.Gold.Value and vName == "GoldPile" then AddESP(v, "Gold " .. (v:GetAttribute("GoldValue") or ""), GoldColor) end
    if Toggles.AutoBreaker and Toggles.AutoBreaker.Value and vName == "ElevatorBreaker" then task.spawn(Breaker, v) end
    
    if Toggles.PromptReach and Toggles.PromptReach.Value and v:IsA("ProximityPrompt") then
        v:SetAttribute("Range", v.MaxActivationDistance)
        v.MaxActivationDistance = v.MaxActivationDistance * 2
    end
    
    if Toggles.Books and Toggles.Books.Value and vName == "LiveHintBook" then AddESP(v, "Library Book", BookColor) end
    
    if Toggles.Snare and Toggles.Snare.Value and vName == "Snare" then
        task.spawn(function()
            local waitTime = 0
    ...
    if Toggles.AntiLava and Toggles.AntiLava.Value and vName == "Lava" then v.CanTouch = false end
    if Toggles.AntiWall and Toggles.AntiWall.Value and vName == "ScaryWall" then
        task.spawn(function()
            task.wait(0.1)
            for _, i in pairs(v:GetChildren()) do if i:IsA("BasePart") then i.CanTouch = false end end
        end)
    ...
    if Toggles.RealBridge and Toggles.RealBridge.Value and vName == "Bridge" and v.CanCollide == false then
    ...
    if Toggles.Entity and Toggles.Entity.Value then
        if vName == "Snare" then
            task.spawn(function()
                local w = 0
                repeat task.wait(0.1); w += 0.1 until v:FindFirstChild("Hitbox") or w > 2
                if v:FindFirstChild("Hitbox") then AddEntityESP(v, "Snare", EntityColor) end
            end)
        end
    end
...
table.insert(Connections, workspace:WaitForChild("CurrentRooms").ChildAdded:Connect(function(room)
    task.spawn(function()
        -- Wait for the engine to finish dumping the 5,000 parts into the room
        task.wait(0.5)
        
        -- Process all parts synchronously, yielding slightly to prevent ANY lag spikes
        ...
        -- Now attach DescendantAdded for late-spawning entities (e.g. Guiding Light, delayed Snares)
        -- This avoids firing the event 5,000 times during room generation!
        table.insert(Connections, room.DescendantAdded:Connect(processPart))
    end)
end))

-- Also process existing rooms in case the script was injected mid-game
for _, room in ipairs(workspace:WaitForChild("CurrentRooms"):GetChildren()) do
    task.spawn(function()
        local descendants = room:GetDescendants()
        for i, v in ipairs(descendants) do
            processPart(v)
...
task.spawn(function()
    pcall(function()
        local EntityModules = ReplicatedStorage:WaitForChild("ClientModules", 5):WaitForChild("EntityModules", 5)
        local SpiderModule = EntityModules:WaitForChild("SpiderJumpscare", 5)
        local spiderFunc = require(SpiderModule)
        local oldSpider
        oldSpider = hookfunction(spiderFunc, function(...)
            if Toggles.AntiTimothy and Toggles.AntiTimothy.Value then
                return
            end
            return oldSpider(...)
        end)
...
table.insert(Connections, workspace.CurrentCamera.ChildAdded:Connect(function(child)
    if Toggles.AntiTimothy and Toggles.AntiTimothy.Value and (child.Name == "SpiderJumpscare" or child.Name == "Timothy" or child.Name == "Spider") then
        task.wait()
        child:Destroy()
    end
end))
