-- ============================================================
--  KAIM v8.7  |  WindUI Edition (Singularity Revision)
--  Filtered Kinematics, Spherecast Triggerbot, Dynamic Bounds
--  Improved Edition: Bug fixes, perf optimizations, full UI
-- ============================================================
...
local Settings = {
    ...
    Visuals = {
        ...
        TracerLines        = false,
        TracerOrigin       = "Bottom",
        TracerColor        = Color3RGB(140, 82, 255),
        DamageNumbers      = false,
...
local function UpdateESP(camPos, screenWidth, screenHeight, visSet, activeCam)
    ...
    -- FIX: cleaner tracerOriginY logic (was nested ternary chain)
    local tracerOriginY
    ...
    for i = 1, #PlayerCache do
        ...
        local cData = CharCache[player]
        if not cData or not cData.HRP.Parent or not cData.Hum or cData.Hum.Health <= 0 then
            HideESP(esp)
...
-- ============================================================
--  WINDUI MENU BUILDING (Singularity v8.7)
-- ============================================================
...
--  [ DASHBOARD ]
-- ============================================================
local HomeWelcome = Tabs.Home:Section({ Title = "Welcome to KAIM v8.7", Box = true, Opened = true })
HomeWelcome:Paragraph({
    Title = "The Singularity Revision",
    Desc  = "v8.7 fixes: math.pow removed (Luau compat), circular velocity ring buffer, staggered GetExtentsSize, font cache, arrow line table, NaN guards, single CharacterAdded conn, THUD hide guard, SafeContainer never falls to workspace, KAIM_LOADED cleared on unload, auto-save on close, and all missing UI sliders added.",
    Icon  = "crown",
})
...
-- FOV Controls (NEW — was missing from original)
local FOVSec = Tabs.Combat:Section({ Title = "FOV Circle", Box = true, Opened = true })
...
IndicatorSec:Colorpicker({ Title = "Tracer Color",  Desc = "Color of tracer lines.",                        Flag="TracerColor",   Default=Color3RGB(140,82,255), Transparency=0, Callback=function(v) Settings.Visuals.TracerColor     = v end })
...
ESPColors:Colorpicker({ Title = "Visible Color",       Desc = "Color when enemy is visible.",              Flag="ESPVisColor",    Default=Color3RGB(140,82,255), Transparency=0, Callback=function(v) Settings.Visuals.VisColor      = v end })
...
Tabs.Home:Select()
WindUI:Notify({ Title = "KAIM v8.7 Singularity", Content = "Initialization complete. Press K to toggle.", Duration = 5, Icon = "shield-check" })
