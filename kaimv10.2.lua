...
local THUD = {
    -- Background layers (outer shadow → main bg → inner edge glow)
...
local function TickAim(camP, sw, sh, dt, fovP, cam)
            ...
            if cd.Hum.Health <= 0 then
                ...
                if part then
                    ...
                    if S.ESP.HUD then
                        ...
                        local nameSz = mMax(13, mFloor(15 * sc))
                        local subSz = mMax(10, mFloor(11 * sc))
                        local bw = mFloor(mMax(280, 60 + mMax(#nameStr, #modeStr + #distStr + 6) * 8) * sc)
                        local acH = mMax(2, mFloor(3 * sc))  -- Accent bar height
                        local barH = mMax(6, mFloor(8 * sc))  -- Health bar height
                        local bh = mFloor((acH + pad + nameSz + 6 + 1 + 6 + barH + 4 + subSz + pad) * 1)
                        local hx = mFloor((sw - bw) * 0.5)
                        local hy
                        if st == "Top Premium" then
                            hy = mFloor(sh * 0.05) - mFloor(ys)
                        else
                            hy = mFloor(sh * 0.82) + mFloor(ys)
                        end

                        -- Shadow (offset soft glow behind)
                        ...
                        -- Top accent bar (color = visible/hidden indicator)
                        setRect(THUD.Ac, V2(bw, acH), V2(hx, hy), ac, 11, nil, ea, true)

                        -- Accent glow (subtle fade beneath accent)
                        ...
                        -- Row 1: Name (left) + Distance (right-ish, after name)
                        local textY = hy + acH + pad
                        setText(THUD.N, nameStr, V2(hx + pad, textY), WHITE, nameSz, 12)
                        THUD.N.Transparency = ea
    ...
    if not showHUD then
        ...
        if thAlpha <= 0.02 then HideTHUD() else
            ...
            THUD.BG.Transparency = 0.9*ea; THUD.Out.Transparency = 0.6*ea
...
local function TickESP(camP, sw, sh, cam, tStart, dt)
    ...
    for i = 1, pCount do
        ...
        if not cd then HideE(e); continue end
        local hrp = cd.HRP; local hum = cd.Hum
        if not hrp or not hum or hum.Health <= 0 then HideE(e); continue end

        -- IsDescendantOf guard — only pcall if we haven't validated this frame
        if not cd._wsValid then
            local ok2, inWs = pcall(hrp.IsDescendantOf, hrp, workspace)
            if not ok2 or not inWs then HideE(e); continue end
            cd._wsValid = true  -- Reset in TickRender pre-pass
        end
...
local function TickRender(dt)
    ...
    for i=1, #PList do
        ...
        if cd and cd.HRP and cd.Hum and cd.Hum.Health > 0 then
            cd._wsValid = false  -- Reset per-frame; ESP will validate once
